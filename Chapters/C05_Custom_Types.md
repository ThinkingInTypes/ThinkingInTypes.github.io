...
The `show_generated_init()` method walks through the field definitions and reconstructs a simplified version of the `__init__` method that would be generated by the decorator.
For each field, it includes a type annotation and default value, and it emits code that assigns constructor parameters to instance attributes.
This generated initializer is what causes instance attributes to appear when we create a `dataclass` instance.
