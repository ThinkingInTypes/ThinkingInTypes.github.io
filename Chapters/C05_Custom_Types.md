...
Now we can see the methods generated by different configurations of `@dataclass`:
...
Adding `frozen=True` produces immutability by restricting instance mutation and enabling hashability.
`__setattr__` and `__delattr__` are defined to prevent modification at runtime.
