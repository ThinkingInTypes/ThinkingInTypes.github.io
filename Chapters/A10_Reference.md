...
This enabled _gradual typing_--code can be partially or fully annotated, and type checkers will treat unannotated parts as dynamically typed (effectively type `Any`)  .
...
You might use `Any` when you genuinely cannot predict the type (e.g.
a function that can return any type depending on usage).
...
Static analyzers use it to understand control flow (e.g.
after a call to a NoReturn function, execution doesn't continue).
...
which is equivalent to the earlier definition .
...
e.g., `user_class: type[BasicUser]` means a class object inheriting from `BasicUser` .
...
This doesn't change runtime behavior, but a type checker will verify that you actually are overriding a base class method and that your method's signature is compatible with the base class version .
...
If you are using a library that isn't annotated, or you cannot modify source code to add annotations (e.g.
C extension modules or third-party code), you can use _stub files_ (`.pyi` files) to provide type information.
