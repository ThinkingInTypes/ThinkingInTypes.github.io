...
In newer Python, the built-in collection classes (list, dict, etc.) and user-defined generics support the shorthand `ClassName[Type]` syntax directly.
...
then `curry_two_arg(func)` returns a function that takes `X` and returns another function which takes `Y` and returns `Z`.
...
We could also achieve a similar result using `ParamSpec` from `typing` (which allows capturing an arbitrary list of parameters),
...
Python 3.11+ provides a `typing.Self` type (PEP 673) that simplifies this pattern by allowing methods to return `Self` (the type of the actual class).
For example:
...
Protocols create interfaces or contracts that can be satisfied by any class, matching Python's dynamic duck typing in a static type checking context.
...
but they are accepted by the `render` function because they fulfill the structural requirements of the `Drawable` protocol.
